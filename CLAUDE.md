...
- Every `jo/games/*` page's static "→ العودة لأدوات الأردن" back-link and was hardcoded to `href="jo/"` — harmless while games were only linked from `/jo/`, but became a real bug once the challenges module was added to every other country hub (`om/`, `ae/`, `sa/`, `us/`, `uk/`): a user reaching a game from `/om/` would hit "back" and land on `/jo/` instead of `/om/` (user-reported). Fixed 2026-09-07 by giving the link `data-i18n="back"` (the same generic key every other tool page on the site already uses) instead of custom Jordan-specific text — this makes both the existing `fixBackLink()` (sets `href` to `getHomeHref()`, i.e. the visitor's actual country hub) and the translation system (sets the visible text) apply automatically, no per-page JS needed. Applied to all 15 `jo/games/*` pages including `wordle/`.
